...
void DrawPlayer(const Game& game, const Player& player, const char* sprite[]);
...
void DrawGame(const Game& game, const Player& player,Shield shields[], int numberOfShields, const AlienSwarm& aliens, const AlienUFO& ufo, const HighScoreTable& table)
{
	if(game.currentState == GS_PLAY || game.currentState == GS_PLAYER_DEAD || game.currentState == GS_WAIT)
	{
		...
		{
			DrawPlayer(game, player, PLAYER_SPRITE);
		}
		else
		{
			DrawPlayer(game, player, PLAYER_EXPLOSION_SPRITE);
		}
...
void ResetShields(const Game&game,Shield shields[], int numberOfShields)
{
	...
	for(int i = 0; i < numberOfShields; i++)
	{
		...
		for(int row = 0; row < SHIELD_SPRITE_HEIGHT; row++)
		{
			strcpy_s(shield.sprite[row], 8, SHIELD_SPRITE[row]);
		}
	}
}
