# -*- coding: utf-8 -*-
"""
Created on Mon Apr 27 23:09:50 2020
...
# margin_img = np.ones(shape=(220, 10, 3))
# combined_image = np.hstack((images, margin_img, np.dstack((grayimages,)*3), margin_img, np.dstack((normalize_image(filteredimages),)*3)))
#
# cv2.imwrite('OriginalGrayFiltered_sobelx.png', (255.0*combined_image).astype(np.uint8))

# input_height, input_width = grayimages.shape
input_height = 256
input_width = 256
def linearcnn_model():
...
def visualize_matrix(M, epoch=1):
    ...
    ax1.plot([-bracket_paddingl, -bracket_paddingl + bracket_b], [hcenter - bracket_d, hcenter - bracket_d], 'k-', lw=2,
             alpha=0.7)
    ax1.plot([-bracket_paddingl, -bracket_paddingl + bracket_b], [hcenter + bracket_d, hcenter + bracket_d], 'k-', lw=2,
             alpha=0.7)

    ax1.plot([wmargin - bracket_paddingr, wmargin - bracket_paddingr], [hcenter - bracket_d, hcenter + bracket_d], 'k-',
             lw=2, alpha=0.7)
    ax1.plot([wmargin - bracket_paddingr - bracket_b, wmargin - bracket_paddingr],
             [hcenter - bracket_d, hcenter - bracket_d], 'k-', lw=2, alpha=0.7)
    ax1.plot([wmargin - bracket_paddingr - bracket_b, wmargin - bracket_paddingr],
             [hcenter + bracket_d, hcenter + bracket_d], 'k-', lw=2, alpha=0.7)
...
combined_image = np.hstack(
    (np.dstack((normalize_image(predicted_img),) * 3), margin_img, np.dstack((normalize_image(filtered_img),) * 3)))
